...
def decoupe_matrice_2_par_3(word):
    ...
    return (im.crop(mot), im.crop(rectangle_hg), im.crop(rectangle_hm), im.crop(rectangle_hd), im.crop(rectangle_bg), im.crop(rectangle_bm), im.crop(rectangle_bd))
...
def calcul_somme_diff_matrice(m1, m2):
    return abs(m1[0] - m2[0]) + abs(m1[1] - m2[1]) + abs(m1[2] - m2[2]) + abs(m1[3] - m2[3]) + abs(m1[4] - m2[4]) + abs(m1[5] - m2[5])
...
mot_choisi = "de" #Peut être remplacé par un input mais il faut modifier la bdd (stocker les valeurs correspondant à cet input)
        ...
        try :
            ...
            for word in racine.iter("word"):
                if (word.attrib.get("searchWord") == mot_choisi):
                    ...
                    tab_score_mots_matrice.append((calcul_moyenne_image(mat[0]),calcul_moyenne_image(mat[1]),calcul_moyenne_image(mat[2]),calcul_moyenne_image(mat[3]),calcul_moyenne_image(mat[4]),calcul_moyenne_image(mat[5])))
...
if cpt_erreur == 0:
    ...
    print("Images analysées avec ", cpt_erreur, " erreurs detectées (les erreurs indiquent que les fichiers dans les archives ont des noms différents de l'image !")
    exit(4)
...
requete = """  (select P.page_id, niveau_gris, point_noir, point_blanc, gamma from page as P, triplet as T where P.page_id = T.page_id AND niveau_gris >= %s and P.page_id != %s order by niveau_gris asc limit 1) 
                union
                (select P.page_id, niveau_gris, point_noir, point_blanc, gamma from page as P, triplet as T where P.page_id = T.page_id AND niveau_gris < %s and P.page_id != %s order by niveau_gris desc limit 1)
                order by abs(niveau_gris - %s)
                limit 1"""
#Connexion à la BDD
conn = connexionBDD()
...
requete = """select P.page_id, point_noir, point_blanc, gamma, haut_gauche, haut_milieu, haut_droite, bas_gauche, bas_milieu, bas_droite, fascicule_id from page as P, matrice as M, triplet as T where P.page_id = T.page_id AND P.page_id = M.page_id"""
    ...
    if not rows:
        ...
        for row in rows:
            if not (row[10] == id_fascicule) : #On exclue le fascicule cible.
                for i in range(len(tab_fichiers_traites)):
                    temp = calcul_somme_diff_matrice(tab_moyennes_matrice[i], (float(row[4]), float(row[5]), float(row[6]), float(row[7]), float(row[8]), float(row[9])))
...
requete = """  (select * from analogie where difference_niveau_gris >= %s and page1 != %s order by difference_niveau_gris asc limit 1) 
                union
                (select * from analogie where difference_niveau_gris < %s and page1 != %s order by difference_niveau_gris desc limit 1)
                order by abs(difference_niveau_gris - %s)
                limit 1"""
#On se replace au début de la BDD.
cursor = setup_curseur(conn)
...
for fichier in stockage_triplets:
    ...
    texte += fichier.replace(id_fascicule, "").replace(".png", "") + " " + str(pn_original_normalise).replace(".",",") + " " + str(pn_normalise).replace(".",",")  + " " + str(pb_original_normalise).replace(".",",") + " " + str(pb_normalise).replace(".",",") + " " + str(gamma_original_normalise).replace(".",",") + " " + str(gamma_normalise).replace(".",",") + "\n"
...
texte = ";score_v1;score_v2;score_v3;=MOYENNE(B:B);=MEDIANE(B:B);=MOYENNE(C:C);=MEDIANE(C:C);=MOYENNE(D:D);=MEDIANE(D:D)\n"
for fichier in stockage_triplets:
    ...
    texte += fichier.replace(id_fascicule, "").replace(".png", "") + ";" + str(score_v1).replace(".",",") + ";" + str(score_v2).replace(".",",") +";" + str(score_v3).replace(".",",") + "\n"
